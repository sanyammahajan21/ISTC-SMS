...
import { columns } from "../cn/Columns";
...
function OldEntryModal() {
  ...
  const handleFormSubmit = () => {
    (async () => {
      try {
        const url = `${showMarksAPI_Registrar}?course=${formData.course}&subject=${formData.subject}&semester=${formData.semesterNo}&currentSemester=${formData.currentSemester}`;
        const response = await axios.get(url, {
          withCredentials: true,
          headers: {
  ...
  const downloadCsvHandler = async () => {
    ...
    try {
      ...
      const url = `${downloadMarksAPI_Registrar}?course=${formData.course}&semester=${formData.semesterNo}&subject=${formData.subject}&currentSemester=${formData.currentSemester}`;
      ...
      if (response.status === 200) {
        ...
        downloadLink.download = `Marks_${formData.course}_Semester_${formData.semesterNo}_${formData.subject}.csv`;
  ...
  return (
    <div className="flex overflow-auto w-screen h-screen bg-lavender font-poppins text-black-blue background">
          ...
          {!showTable ? (
            ...
            <>
              ...
              <div className="container max-w-[75rem]  ">
                <OldEntry studentsdata={students} index={formData.index-1} subject={formData.subject} semester={formData.semesterNo} />
              </div>
            </>
          )}
        </div>

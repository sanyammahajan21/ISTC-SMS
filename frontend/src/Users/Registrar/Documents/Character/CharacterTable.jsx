...
import { downloadCharacterAPI_Registrar } from "../../../../Assets/backendAPIs";
import { toast } from "react-toastify";

const handleDownloadClick = async (rowData, course) => {
    try {
      // Create the URL with query parameters
      const url = `${downloadCharacterAPI_Registrar}?course=${course}&rollNo=${rowData.rollNo}`;
      ...
      if (response.status === 200) {
        ...
        downloadLink.download = `${rowData.rollNo}_Character_Certificate.pdf`;
        ...
        toast.success(" Character certificate downloaded");
      }
    } catch (error) {
      toast.warning(
...
const columns1 = [
    ...
    },
    {
        ...
        header: "Download Character Certificate",
        cell: ({ row ,course }) => (
            <Button
                variant="ghost"
...
const CharacterTable = ({ data ,course }) => {
    return (
        <DataTable
            columns={columns1}
            data={data}
